...
docker-deploy: docker-push
	...
	ssh vps "\
		...
		docker run -d --name ticketmet --restart unless-stopped -p 127.0.0.1:11200:8080 -e TICKETMASTER_API_KEY='$$TICKETMASTER_API_KEY' -e WEBAUTHN_RP_ID=ticketmet.jessyfal04.dev -e WEBAUTHN_ORIGINS=https://ticketmet.jessyfal04.dev -e APP_BASE_URL=https://ticketmet.jessyfal04.dev -e SMTP_HOST=10.66.66.1 -e SMTP_PORT=25 -e SMTP_FROM=ticketmet@jessyfal04.dev -e SETLISTFM_API_KEY='$$SETLISTFM_API_KEY' -v /opt/ticketmet/data:/app/data $(IMAGE); \
